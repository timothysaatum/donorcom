...
Revision ID: ba70550ef778
Revises:
Create Date: 2025-07-26 13:10:40.913691
...
def upgrade() -> None:
    ...
    # 1. Create facilities table WITHOUT foreign key to users
    ...
    # 3. Create users table WITHOUT foreign key to facilities
    ...
    op.create_foreign_key(
        'fk_facilities_facility_manager_id',
        'facilities', 'users',
        ['facility_manager_id'], ['id'],
        ondelete='CASCADE'
    )
    ...
    # 4. Create blood_banks table (depends on facilities and users)
    ...
    # 5. Create blood_requests table (depends on facilities and users)
    ...
    # 6. Create dashboard_daily_summary table (depends on facilities)
    ...
    # 7. Create blood_inventory table (depends on blood_banks and users)
    ...
    # 8. Create blood_distributions table (depends on blood_inventory, users, blood_banks, facilities)
    ...
    # 9. Create track_states table (depends on blood_distributions and users)
    ...
    with op.batch_alter_table('track_states', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_track_states_id'), ['id'], unique=True)
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('track_states', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_track_states_id'))
    ...
    op.drop_constraint('fk_facilities_facility_manager_id', 'facilities', type_='foreignkey')
