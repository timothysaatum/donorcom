...
# Conditional imports for Lambda
try:
...
# Conditional admin imports (admin may not work well in Lambda)
...
if not IS_LAMBDA:
    try:
        ...
        print("Admin modules not found - running without admin interface")
        Admin = None
...
def create_application() -> FastAPI:
    app = FastAPI(
        ...
        description=settings.PROJECT_DESCRIPTION,
        version=settings.VERSION,
        docs_url=settings.DOCS_URL if not IS_LAMBDA else None,  # Disable docs in Lambda
        redoc_url=None
    )
    ...
    # Only add admin in development/local environment
    if not IS_LAMBDA and Admin is not None:
        admin = Admin(app, engine, base_url="/admin")
        admin.add_view(UserAdmin)
        admin.add_view(FacilityAdmin)
        admin.add_view(BloodBankAdmin)
    ...
    # Custom OpenAPI config for Swagger (only in development)
    if not IS_LAMBDA:
        def custom_openapi():
            if app.openapi_schema:
                return app.openapi_schema
    ...
    async def health_check():
        return {"status": "healthy", "environment": "lambda" if IS_LAMBDA else "development"}
...
# Lambda handler - only created if mangum is available
