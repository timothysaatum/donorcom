...
class BloodInventoryService:
    ...
    async def create_blood_unit(
        self, blood_data: BloodInventoryCreate, blood_bank_id: UUID, added_by_id: UUID
    ) -> BloodInventory:
        ...
        await self.db.refresh(
            new_blood_unit, attribute_names=["blood_bank", "added_by"]
        )
        ...
        try:
            ...
            if new_blood_unit.blood_bank and new_blood_unit.blood_bank.facility_id:
                await refresh_facility_dashboard_metrics(
                    self.db, new_blood_unit.blood_bank.facility_id
                )
    ...
    async def get_facilities_with_available_blood(
        ...
        """
        # Validate blood type and product only if they are provided
        if blood_type or blood_product:
            self._validate_blood_attributes(blood_type, blood_product)

        # Build base conditions - always check for positive quantity and non-expired
        base_conditions = [
        ...
        query = (
            select(Facility.id.label("facility_id"), Facility.facility_name)
            .distinct()
            .select_from(Facility)
            .join(BloodBank, Facility.id == BloodBank.facility_id)
            .join(BloodInventory, BloodBank.id == BloodInventory.blood_bank_id)
        ...
        return PaginatedResponse(
            items=[
                FacilityWithBloodAvailability(**facility) for facility in facilities
            ],
    ...
    async def _validate_blood_attributes(
        self, blood_type: Optional[str], blood_product: Optional[str]
    ):
        ...
        if blood_type:
            validation_data["blood_type"] = blood_type
        else:
            validation_data["blood_type"] = "A+"  # Default for validation

        if blood_product:
            validation_data["blood_product"] = blood_product
        else:
            validation_data["blood_product"] = "Whole Blood"  # Default for validation

        try:
            BloodInventoryCreate(
                **validation_data, quantity=1, expiry_date=datetime.now().date()
            )
        except ValueError as e:
            # Only raise error if the provided values are invalid
            error_msg = str(e)
            if blood_type and "Blood type must be one of" in error_msg:
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail=f"Invalid blood type: {blood_type}",
                )
            if blood_product and "Blood product must be one of" in error_msg:
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail=f"Invalid blood product: {blood_product}",
                )
    ...
    async def update_blood_unit(
        self, blood_unit_id: UUID, blood_data: BloodInventoryUpdate
    ) -> BloodInventory:
        """Update a blood unit"""
    ...
    async def get_paginated_blood_units(
        ...
        current_user_blood_bank_id: Optional[UUID] = None,  # Add this parameter
    ) -> PaginatedResponse[BloodInventory]:
        """
        Get paginated blood units with comprehensive filtering and sorting
        Always filters by the user's blood bank if current_user_blood_bank_id is provided
        """
        # Build base query with optimized joins
        query = select(BloodInventory).options(
            joinedload(BloodInventory.blood_bank), joinedload(BloodInventory.added_by)
        )

        # Apply filters - always include the user's blood bank filter if provided
        conditions = []

        # This is the key change - always filter by user's blood bank if provided
        if current_user_blood_bank_id:
            conditions.append(
                BloodInventory.blood_bank_id == current_user_blood_bank_id
            )
        elif (
            blood_bank_id
        ):  # Fallback to provided blood_bank_id if no user bank provided
    ...
    async def get_blood_units_by_bank(
        self, blood_bank_id: UUID, pagination: Optional[PaginationParams] = None
        ...
        if pagination:
            return await self.get_paginated_blood_units(
                pagination=pagination, blood_bank_id=blood_bank_id
            )
    ...
    async def get_expiring_blood_units(
        self, days: int = 7, pagination: Optional[PaginationParams] = None
    ) -> List[BloodInventory] | PaginatedResponse[BloodInventory]:
    ...
    async def get_blood_units_by_type(
        self, blood_type: str, pagination: Optional[PaginationParams] = None
    ) -> List[BloodInventory] | PaginatedResponse[BloodInventory]:
        """Get blood units by type with optional pagination"""
        if pagination:
            return await self.get_paginated_blood_units(
                pagination=pagination, blood_type=blood_type
            )
