...
from app.models.request_model import RequestStatus, ProcessingStatus
...
async def list_my_requests(
    ...
    logger.info(
        ...
        extra={"event_type": "my_requests_access", "current_user_id": current_user_id},
    )
    ...
    try:
        ...
        if duration_ms > 1000:  # More than 1 second
            log_performance_metric(
                ...
                duration_seconds=duration_ms / 1000,
                additional_metrics={"slow_query": True, "result_count": len(requests)},
            )
...
async def list_facility_requests(
    request: Request,
    option: Optional[RequestDirection] = Query(
        RequestDirection.ALL,
        description="Filter requests by direction: 'received', 'sent', or 'all'",
    ),
            ...
            log_performance_metric(
                ...
                additional_metrics={
                    ...
                    "filters_applied": {
                        ...
                        "processing_status": (
                            processing_status.value if processing_status else None
                        ),
                    },
                },
            )
...
async def get_request_statistics(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(
        require_permission("facility.manage", "laboratory.manage")
    ),
):
    ...
    try:
        ...
        if duration_ms > 2000:  # More than 2 seconds
            log_performance_metric(
                ...
                duration_seconds=duration_ms / 1000,
                additional_metrics={"slow_query": True, "statistics_generated": True},
            )
...
async def get_request_group(
    ...
    try:
        ...
        if group.master_request.requester_id != current_user.id:
            ...
            raise HTTPException(
                status_code=403, detail="Not authorized to view this request group"
            )
    ...
    try:
        ...
        is_same_facility = (
            ...
            or (
                ...
                and blood_request.facility_id == current_user.facility.id
            )
        )
...
async def list_requests_by_status(
    ...
    try:
        ...
        user_requests = [
            req for req in all_requests if req.requester_id == current_user.id
        ]
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve requests by status"
        )


# Routes for facility managers/staff
@router.get(
    "/facility/{facility_id}/requests", response_model=List[BloodRequestResponse]
)
async def list_facility_requests_by_id(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve facility requests"
        )
...
async def respond_to_request(
    ...
    logger.info(
        ...
        extra={
            ...
            "request_id": req_id,
            "new_status": (
                response_data.request_status.value
                if response_data.request_status
                else None
            ),
    ...
    try:
        ...
        old_values = {
            ...
            "processing_status": blood_request.processing_status,
            "fulfilled_by_id": (
                str(blood_request.fulfilled_by_id)
                if blood_request.fulfilled_by_id
                else None
            ),
            "notes": blood_request.notes,
        }
        ...
        new_values = {
            ...
            "processing_status": updated_request.processing_status,
            "fulfilled_by_id": (
                str(updated_request.fulfilled_by_id)
                if updated_request.fulfilled_by_id
                else None
            ),
            "notes": updated_request.notes,
        }
...
async def cancel_blood_request(
    ...
    cancellation_reason: Optional[str] = Query(
        None, description="Reason for cancelling the request"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(
        require_permission(
...
# =============================================================================
# DISTRIBUTION-RELATED ENDPOINTS FOR REQUESTS
# =============================================================================


@router.get("/{request_id}/distributions")
async def get_request_distributions(
    request_id: UUID,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(
    ...
    try:
        ...
        from app.schemas.distribution_schema import BloodDistributionResponse
        ...
        if not is_requester and not is_same_facility:
            log_security_event(
                event_type="unauthorized_request_distributions_access",
                details={
                    "reason": "not_authorized",
                    "request_id": req_id,
                },
            ...
            raise HTTPException(
                status_code=403,
                detail="Not authorized to view distributions for this request",
            )

        # Get distributions for the request
        ...
        log_audit_event(
            event_type="request_distributions_retrieved",
            table_name="blood_distributions",
            record_id=req_id,
            details={
        ...
        logger.info(
            "Request distributions retrieved successfully",
            extra={
                "event_type": "request_distributions_retrieved",
                "current_user_id": current_user_id,
                "request_id": req_id,
        ...
        logger.error(
            "Failed to retrieve request distributions",
            extra={
                "event_type": "request_distributions_error",
                "current_user_id": current_user_id,
                "request_id": req_id,
                "error": str(e),
                "duration_ms": duration_ms,
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve distributions for request"
        )
...
@router.get("/facility-patterns/{facility_id}")
async def get_facility_request_patterns(
    ...
    """Get request patterns for analytics - optimized for performance."""
    try:
        service = BloodRequestService(db)
        patterns = await service.get_facility_request_patterns(facility_id, days_back)

        logger.info(
            "Facility request patterns retrieved",
            extra={
                "event_type": "facility_patterns_retrieved",
        ...
        logger.error(f"Failed to get facility patterns: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="Failed to retrieve facility patterns"
        )


@router.get("/facility-collaboration/{facility_id}")
async def get_facility_collaboration_stats(
    ...
    try:
        service = BloodRequestService(db)
        stats = await service.get_facility_collaboration_stats(facility_id)

        logger.info(
            "Facility collaboration stats retrieved",
            extra={
                "event_type": "collaboration_stats_retrieved",
                "facility_id": str(facility_id),
                "user_id": str(current_user.id),
            },
        )
        ...
        logger.error(f"Failed to get collaboration stats: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="Failed to retrieve collaboration statistics"
        )


@router.get("/between-facilities/{source_facility_id}/{target_facility_id}")
async def get_requests_between_facilities(
    ...
    """Get requests between specific facilities - optimized query."""
    try:
        service = BloodRequestService(db)
        requests = await service.get_requests_between_facilities(
        ...
        logger.error(
            f"Failed to get requests between facilities: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail="Failed to retrieve requests between facilities"
        )
