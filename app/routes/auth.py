        ...
        if not auth_success:
            if user and hasattr(user, 'is_verified') and not user.is_verified:
                background_tasks.add_task(
                    ...
                    email, user.verification_token if hasattr(user, 'verification_token') else None
                )
...
async def refresh_token(
    ...
    """Enhanced refresh token endpoint with session management"""
    start_time = time.time()
    ...
    try:
        ...
        refresh_token_record = await TokenManager.validate_refresh_token(db, refresh_token)
        ...
        # Create new tokens with session reference
    ...
    try:
        ...
        for session in sessions:
            session_data.append({
                ...
                "location": f"{session.city}, {session.country}" if session.city and session.country else "Unknown",
