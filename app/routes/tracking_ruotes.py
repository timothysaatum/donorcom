    ...
    with LogContext(
        ...
        logger.info(
            ...
            extra={
                ...
                "blood_distribution_id": str(track_data.blood_distribution_id) if track_data.blood_distribution_id else None,
        ...
        try:
            ...
            log_audit_event(
                ...
                new_values={
                    ...
                    "blood_distribution_id": str(new_track.blood_distribution_id) if new_track.blood_distribution_id else None
                },
                user_id=str(current_user.id)
            )
...
async def get_track_states_by_request(
    request_id: UUID = Path(..., description="The ID of the blood request to retrieve track states for"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_permission(
        "facility.manage",
        "laboratory.manage",
...
async def get_track_states_for_distribution(
    tracking_number: str = Path(..., description="The tracking number of the distribution to get states for"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_permission(
        "facility.manage",
        "laboratory.manage",
...
async def update_track_state(
    ...
    with LogContext(
        ...
        try:
            ...
            # Get existing track state for audit logging
            ...
            updated_track = await track_service.update_track_state(track_state_id, track_data, current_user.id)
            ...
            response = TrackStateDetailResponse(
                **TrackStateResponse.model_validate(updated_track, from_attributes=True).model_dump(),
                created_by_name=updated_track.created_by.last_name if updated_track.created_by else None
            )
    ...
    with LogContext(
        ...
        try:
            ...
            old_values = {
                ...
                "notes": existing_track.notes,
                "blood_request_id": str(existing_track.blood_request_id),
                "blood_distribution_id": str(existing_track.blood_distribution_id) if existing_track.blood_distribution_id else None
            }
