...
ENABLE_FILE_LOGGING = (
    os.getenv("ENABLE_FILE_LOGGING", "false" if IS_SERVERLESS else "true").lower()
    == "true"
)
...
class ApplicationLogger:
    ...
    def _setup_file_handlers(self, formatter):
        ...
        app_handler = RotatingFileHandler(
            f"{LOG_DIR}/app.log", maxBytes=10_000_000, backupCount=10  # 10MB
        )
        ...
        error_handler = TimedRotatingFileHandler(
            f"{LOG_DIR}/error.log", when="midnight", interval=1, backupCount=30
        )
        ...
        perf_handler = RotatingFileHandler(
            f"{LOG_DIR}/performance.log", maxBytes=5_000_000, backupCount=5
        )
        ...
        access_handler = TimedRotatingFileHandler(
            f"{LOG_DIR}/access.log", when="midnight", interval=1, backupCount=30
        )
...
def log_function_call(
    include_args: bool = False, include_result: bool = False, level: str = "DEBUG"
):
    """
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            logger.log(
                log_level, f"Starting {func.__name__}", extra={"extra_fields": log_data}
            )
            ...
            try:
                ...
                if include_result and result is not None:
                    ...
                    if (
                        "password" not in result_str.lower()
                        and "token" not in result_str.lower()
                    ):
...
def log_security_event(
    ...
    log_data = {
        ...
        "severity": (
            "high"
            if event_type
            in ["failed_login_attempt", "unauthorized_access", "token_abuse"]
            else "medium"
        ),
    }
    ...
    security_logger.info(
        f"Security event: {event_type}", extra={"extra_fields": log_data}
    )
...
def log_performance_metric(
    ...
    perf_logger.info(
        f"Performance metric: {operation}", extra={"extra_fields": log_data}
    )
...
def log_api_access(
    ...
    access_logger.info(
        f"{method} {path} - {status_code}", extra={"extra_fields": log_data}
    )
