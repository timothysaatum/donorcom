...
import httpx
>>>>>>> c2e82ddf38ad9071cef41d7bd3cd8d9765a4eef0
from sqlalchemy.ext.asyncio import AsyncSession
# from sqlalchemy.future import select
from datetime import datetime, timedelta, timezone
...
class TestEmailVerification:
    ...
    async def test_verify_email_success(self, sample_user):
        ...
        with patch('app.dependencies.get_db') as mock_get_db, \
            ...
            async with httpx.AsyncClient(app=self.app, base_url="http://test") as client:
                response = await client.get(f"/users/verify-email?token={valid_token}")
    ...
    async def test_verify_email_invalid_token(self):
        ...
        with patch.dict('os.environ', {'SECRET_KEY': 'test-secret', 'ALGORITHM': 'HS256'}):
            async with httpx.AsyncClient(app=self.app, base_url="http://test") as client:
                response = await client.get("/users/verify-email?token=invalid_token")
